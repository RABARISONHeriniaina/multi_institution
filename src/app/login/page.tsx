...
import { setCookie, getCookie, deleteCookie, isAuthenticated } from '@/lib/cookies';
import { getRedirectUrlByAuthenticatedUser } from '@/lib/utils';
...
export default function LoginForm() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setCookie('auth_refresh_token', refresh_token, 30)
      ...
      try {
        ...
        const institutionResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/institution`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        if (institutionResponse.ok) {
          ...
          setCookie('institution_data', JSON.stringify(institutionData), 7)
        }

      } catch (additionalError) {
        console.warn('Failed to fetch additional user data:', additionalError)
      }

      window.location.href = getRedirectUrlByAuthenticatedUser();
